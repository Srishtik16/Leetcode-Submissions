...
public:
    ...
    int recurse(int index, string &s) {
        ...
        int ans = 1;
        for(int i = index + 1; i < (int)s.size(); i += 2) {
            if((i - index + 1) & 1) {
                continue;
            }
            int hash1 = hasher.substring_hash(index, index + (i - index + 1) / 2);
            int hash2 = hasher.substring_hash(index + (i - index + 1) / 2, index + (i - index + 1));
            if(hash1 == hash2) {
                ans = max(ans, recurse(index + (i - index + 1) / 2, s) + 1);
            }
        }
        return dp[index] = ans;
    }
    int deleteString(string s) {
        // dp[i] = Answer for ith suffix
        hasher.build(s);
